...
const EditMembeshipForm = ({ control, errors }) => {
  const {
    fields: criteriaFields,
    append: criteriaAppend,
    remove: criteriaRemove,
  } = useFieldArray({ name: 'criterias', control });

  const {
    fields: benefitFields,
    append: benefitAppend,
    remove: benefitRemove,
  } = useFieldArray({ name: 'benefits', control });
  ...
  return (
    <div className=" bg-grey-200 border-radius py-5 px-5">
      <DoctTypography
        variant="h6"
        className="border-radius p-3 text-grey-600 bg-white font-weight-regular"
      >
        Membership Information
      </DoctTypography>
      <div className="d-flex justify-content-center align-item-center bg-grey-100">
        <div className="form-body-container my-5">
          ...
          <div className="mt-4">
            ...
            <span className="cursor-pointer" onClick={() => benefitAppend({ name: '' })}>
              <DoctTypography
                variant="subtitle2"
                className="text-primary d-flex align-items-center"
              >
...
export default EditMembeshipForm;
